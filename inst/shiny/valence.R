...
valence_ui <- function(id) {
    ...
    tags$table(id = "inputs-table", style = "width: 100%",
        tags$tr(tags$td(style = "width: 90%", uiOutput(ns("selectValenceUI"))),
            tags$td(style = "padding-left: 10px; width: 10%",
                uiOutput(ns("loadValenceUI")))), tags$tr(tags$td(style = "width: 90%",
            uiOutput(ns("methodValenceUI"))), tags$td(style = "width: 10%",
...
valence_server <- function(input, output, session) {
    ...
    myvals <- reactiveValues(valenceList = list_valence_shifters,
        choices = names(list_valence_shifters), selected = NULL,
        useValence = FALSE, method = "Bigrams", choicesMethod = c("Unigrams",
            "Bigrams", "Clusters"))
        ...
        if ("x" %in% colnames(df) && (("y" %in% colnames(df)) ||
            ...
            if (newValenceFileName %in% existingValenceNames) {
                showModal(modalDialog(title = "Warning",
                  paste("A set of valence shifters with the name: '",
                    newValenceFileName, "' already exists.")))
            } else {
                x <- list(data.table::data.table(df))
                names(x) <- newValenceFileName
                myvals$valenceList <- c(myvals$valenceList,
                  x)
                myvals$choices <- names(myvals$valenceList)
                selected <- newValenceFileName
                showModal(modalDialog(title = "Success",
                  paste("Valence shifters with the name: '",
                    newValenceFileName, "' added to the list of valence shifters.")))
            }
        } else {
    ...
    output$selectValenceUI <- renderUI({
        selectizeInput(inputId = ns("selectValence"),
            label = "Select or upload valence shifters",
            choices = c("none", myvals$choices), selected = "en",
            multiple = FALSE)
    })

    output$loadValenceUI <- renderUI({
        dropdownButton(tags$h4("Upload valence shifters"),
            tags$table(id = "inputs-table", style = "width: 80%",
                tags$tr(tags$td(style = "width: 80%",
                  fileInput(inputId = ns("valenceUpload"),
                    label = "Choose .csv file", multiple = FALSE,
                    accept = c("text/csv", "text/comma-separated-values,text/plain",
                      ".csv"))), tags$td(style = "width: 10%",
                  div(class = "form-group shiny-input-container",
                    actionButton(inputId = ns("valenceHelpButton"),
    ...
    output$methodValenceUI <- renderUI({
        radioGroupButtons(inputId = ns("valenceMethod"),
            label = "Valence shifting", choices = c("Unigrams"),
            justified = TRUE)
    })

    output$methodValenceHelpUI <- renderUI({
        div(class = "form-group shiny-input-container",
            style = "margin-top: 25px", actionButton(inputId = ns("valenceMethodHelpButton"),
                label = NULL, icon = icon("question")))
    })

    observeEvent(input$valenceHelpButton, {
        showModal(modalDialog(title = "Upload valence shifters",
    ...
    observeEvent(input$valenceMethodHelpButton, {
        showModal(modalDialog(title = "Valence shifting method",
